...
echo [1/8] Pulling Docker image '%imagename%'...
docker pull %imagename%
IF ERRORLEVEL 1 (
    echo Error: Failed to pull Docker image '%imagename%'. Check the image name and Docker connectivity.
    exit /b 1
)
...
docker run -d --rm --name %temp_container_name% %imagename% tail -f /dev/null
IF ERRORLEVEL 1 (
    echo Error: Failed to start temporary Docker container '%temp_container_name%'.
    exit /b 1
)
...
docker export %temp_container_name% > "%temp_tar_path%"
IF ERRORLEVEL 1 (
    echo Error: Failed to export Docker container '%temp_container_name%'. Check permissions and available disk space in '%TEMP%'.
    docker stop %temp_container_name% > nul
    exit /b 1
)
...
docker stop %temp_container_name% > nul
IF ERRORLEVEL 1 (
    echo Warning: Failed to stop temporary Docker container '%temp_container_name%'. It might have already stopped or encountered an issue.
) else (
...
IF ERRORLEVEL 0 (
    ...
    IF ERRORLEVEL 1 (
        ...
        IF ERRORLEVEL 1 (
            ...
            echo Initial setup commands (apt) executed.
        )
    )
) else (
    ...
    IF ERRORLEVEL 0 (
        ...
        IF ERRORLEVEL 1 (
            ...
            IF ERRORLEVEL 1 (
                ...
                echo Initial setup commands (apk) executed.
            )
        )
    ) else (
...
echo --- Success ---
echo WSL distribution '%DistroName%' created successfully from Docker image '%imagename%'.
...
:Usage
@echo
@echo Imports a Docker image into WSL as a new distribution.
@echo .
@echo Usage: %~n0 /image <docker_image_name> [options]
@echo .
@echo Required:
@echo   /image <name>    Specifies the Docker image to import (e.g., ubuntu:latest).
@echo .
@echo Options:
